...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(discord_cfg) = cfg.discord {
        ...
        let mut client = if let Some(ref proxy) = discord_cfg.proxy {
            // serenity only applies the proxy when the ratelimiter is disabled —
            // the ratelimiter path builds requests with proxy=None. So the two
            // are intentionally coupled: proxy implies ratelimiter_disabled.
            info!(proxy = %proxy, "discord adapter using API proxy (ratelimiter disabled)");
