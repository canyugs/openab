...
- **Status:** Proposed — text-confirmed local ACP loop live-validated; spoken output/TTS pending
- **Date:** 2026-07-12
...
| Local Discord/Groq/Claude readiness | **Passed; text-confirmed local action loop passed; Voice Brief deployed** | Helm revision 11 runs `localhost:5555/openab:claude-voice-brief-c96ea57` at digest `sha256:1979f2dcea61ac0013ba1817428fe2e78c57906e83de11f71c8c9669e510d618`, `1/1` ready with zero restarts. The preceding revision passed a real utterance through proposal, confirmation, Claude ACP action, and text result. Revision 11 adds the bounded Voice Brief completion path; live result-brief playback is pending because `TTS_API_KEY` is absent. Target-agent handoff also remains pending. |
...
| Voice intent broker | **Local route implemented and live-validated; delegation validation pending** | With explicit opt-in and `default_to_local = true`, every non-empty final transcript from the session operator received while no confirmation is pending becomes a local raw-task candidate unless it explicitly addresses exactly one configured target. The real voice-to-text-proposal-to-confirmation-to-local-ACP action loop passed on 2026-07-13. Explicit configured-target delegation remains implemented but not live-validated. |
| Spoken confirmation and TTS | **Implemented; result brief added; live provider/Voice validation pending** | Final STT from the bound operator handles yes/no/correction through the same pending state as text. Opt-in OpenAI-compatible TTS plays bounded broker feedback. Confirmed local ACP turns now return typed completion to the Voice action and expose only an explicit bounded `voice_summary`; arbitrary final output is never a TTS fallback. |
| Realtime/Live and result observation | **Local ACP completion observation implemented; delegated observation and Realtime pending** | A one-shot completion observer follows the exact local action through canonical Discord delivery. Target-agent thread polling and Realtime/Live remain later slices. |
