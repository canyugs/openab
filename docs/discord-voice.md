...
| Local Kubernetes runtime smoke | **LLM-first local action loop passed; Voice Brief deployed.** Helm revision 11 runs `localhost:5555/openab:claude-voice-brief-c96ea57` at digest `sha256:1979f2dcea61ac0013ba1817428fe2e78c57906e83de11f71c8c9669e510d618`, `1/1` ready with zero restarts and `default_to_local = true`. The preceding revision passed a real Voice STT proposal, confirmation, Claude ACP action, and text result. Revision 11 adds bounded result briefs but reports `voice_tts_ready=false` because the local Secret lacks `TTS_API_KEY`; live audible playback and target handoff remain pending. |
